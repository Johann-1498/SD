...
No special modifications are needed to run the 'Server.py' file.

By default the server listens on:
- `DNS_IP=127.0.0.1`
- `DNS_PORT=53`

You can override these with environment variables. This is useful during development to avoid privileged port 53.
...
Bash (Linux/macOS):
```bash
DNS_IP=127.0.0.1 DNS_PORT=5053 python3 Server.py
```

If you use port `53`, run with the necessary privileges and ensure no other process is already bound to that port.
...
Windows example with `nslookup`:
```powershell
nslookup -type=A -port=5053 xyz.com 127.0.0.1
```
...
# Scripts visuales (Windows PowerShell)

Servidor visual:
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\start-server.ps1 -Ip 127.0.0.1 -Port 5053
```

Cliente visual:
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\test-client.ps1 -ServerIp 127.0.0.1 -Port 5053 -Domain xyz.com
```
