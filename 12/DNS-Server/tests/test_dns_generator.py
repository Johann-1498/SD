...
class TestDnsGen(unittest.TestCase):
    ...
    def test_valid_request_no_records_response(self):
        ...
        """
        dns_request = b'\x2a\xa1\x01\x20\x00\x01\x00\x00\x00\x00\x00\x01\x04xykz\x03com\x00\x00\x01\x00\x01'
        d = DNSGen(dns_request)
        resp = d.make_response()

        # Same transaction ID and one question answered with NXDOMAIN (rcode=3)
        self.assertEqual(dns_request[0:2], resp[0:2])
        self.assertEqual(resp[3:4], b"\x03")
        self.assertEqual(resp[4:6], b"\x00\x01")
        self.assertEqual(resp[6:8], b"\x00\x00")
        self.assertEqual(resp[12:], dns_request[12:])

    def test_malformed_request_format_error(self):
        """
        Create a malformed query missing qtype/qclass after the root label.
        Must return format error (rcode=0001) and no answer records.
        """
        dns_request = b'\xaa\xbb\x01\x20\x00\x01\x00\x00\x00\x00\x00\x00'
        d = DNSGen(dns_request)
        resp = d.make_response()

        self.assertEqual(dns_request[0:2], resp[0:2])
        self.assertEqual(resp[3:4], b"\x01")
        self.assertEqual(resp[6:8], b"\x00\x00")
