...
def parse_server_endpoint(value, default_port):
    ...
    if ":" in raw:
        ...
        try:
            ...
            return None, None
        if not validate_ip(ip) or parsed_port < 0 or parsed_port > 65535:
            return None, None
        return ip, parsed_port
    if validate_ip(raw):
...
def main():
    ...
    # Communications
    pref_server_endpoint = ""
    logging.info(f'Client: starting to communicate')
    try:
        packet = receive_packet(sourceSock)
        pref_server_endpoint = payload_from_packet(packet).decode()
        logging.info(f'Client: received {pref_server_endpoint}')
    except:
    ...
    if pref_server_endpoint == "":
        logging.info(f'Load Balancer could not connect to any servers, ty again later.')
        exit()

    pref_server_ip, pref_server_port = parse_server_endpoint(pref_server_endpoint, port)
    if pref_server_ip is None:
        logging.info(f'Load Balancer returned invalid endpoint: {pref_server_endpoint}')
        exit()

    # Client setup
    logging.info(f'Client: starting socket to prefered server')
    sourceSock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    contactSock = (pref_server_ip, pref_server_port)
    sourceSock.connect(contactSock)
    ...
    try:
        ...
        while not flag:
            ...
            logging.info(f'RECV   {ack_num_from_packet(packet)} {seq_num_from_packet(packet)+payload_len} ACK={"Y "if True else "N"} SYN={"Y" if False else "N"} FIN={"Y" if False else "N"}')
            push_bytes_to_file(webpage,payload)

            if not fin_flag_from_packet(packet):
                send_packet(sourceSock, form_packet(ack_num_from_packet(packet), seq_num_from_packet(packet)+payload_len,b'',ack=True))
            else:
                flag = True
                send_packet(sourceSock, form_packet(ack_num_from_packet(packet), seq_num_from_packet(packet)+payload_len,b'',ack=True, fin=True))
            logging.info(f'SEND   {ack_num_from_packet(packet)} {seq_num_from_packet(packet)+payload_len} ACK={"Y "if True else "N"} SYN={"Y" if False else "N"} FIN={"Y" if True else "N"}')
    except:
        logging.info(f'Client: stopping...')
    finally:
    # Close socket
