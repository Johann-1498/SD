...
def startServer(server, port):
    ...
    try:
        ...
        logging.error(f'Bind failed. Error: {msg}')
        try:
...
class ClientThread(threading.Thread):
    ...
    def run(self):
        ...
        if len(packet) < PacketSizes.HEADER.value:
            logging.info(f'ClientThread: received empty/incomplete packet from {self.ip}, closing connection.')
        ...
        for x in range(self.num_packets):
            ...
            if x<(self.num_packets-1):
                ...
                packet = receive_packet(self.socket)
                if len(packet) < PacketSizes.HEADER.value:
                    logging.info(f'ClientThread: client {self.ip} closed connection early.')
                    break
                ...
                packet = receive_packet(self.socket)
                if len(packet) < PacketSizes.HEADER.value:
                    logging.info(f'ClientThread: client {self.ip} closed connection at final ack.')
                    break
                logging.info(f'RECV   {seq_num_from_packet(packet)} {ack_num_from_packet(packet)} ACK={"Y "if ack_flag_from_packet(packet) else "N"} SYN={"Y" if syn_flag_from_packet(packet) else "N"} FIN={"Y" if fin_flag_from_packet(packet) else "N"}')
