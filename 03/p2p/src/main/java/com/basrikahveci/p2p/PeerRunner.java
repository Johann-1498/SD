    ...
    public CommandResult handleCommand(final String command) {
        ...
        try {
            if (command.equals("ping")) {
                ...
                handle.connect(hostToConnect, portToConnect)
                        .whenComplete(new ConnectFutureListener(hostToConnect, portToConnect));
                ...
                if (tokens.length >= 3) {
                    handle.sendText(tokens[1], tokens[2]);
                } else {
                ...
                final String[] tokens = command.split(" ", 3);
                if (tokens.length >= 3) {
                    handle.sendFile(tokens[1], tokens[2]);
                } else {
                    result = INVALID_COMMAND;
                }
