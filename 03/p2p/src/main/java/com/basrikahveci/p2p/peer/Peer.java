...
public class Peer {
    ...
    public void sendText(String targetPeerName, String text) {
        Connection connection = connectionService.getConnection(targetPeerName);
        if (connection != null) {
            connection.send(new com.basrikahveci.p2p.peer.network.message.TextMessage(config.getPeerName(), text));
            LOGGER.info("Sent text message to {}", targetPeerName);
        } else {
            LOGGER.error("Not connected to {}", targetPeerName);
            System.err.println("Error: Not connected to " + targetPeerName);
        }
    }

    public void sendFile(String targetPeerName, String filePath) {
        Connection connection = connectionService.getConnection(targetPeerName);
        if (connection != null) {
            ...
            try {
                ...
                connection.send(new com.basrikahveci.p2p.peer.network.message.FileMessage(config.getPeerName(),
                        file.getName(), data));
                LOGGER.info("Sent file {} to {}", file.getName(), targetPeerName);
