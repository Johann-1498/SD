...
## Chat Server with Audit - NUEVA IMPLEMENTACIÓN

Servidor chat con broadcast a todos los clientes y auditoría completa.

### Archivos nuevos:
- `chat-server.py` - Servidor multi-cliente con broadcast y auditoría
- `chat-client.py` - Cliente interactivo con username

### Ejecutar Servidor:
```bash
python chat-server.py 127.0.0.1 65432
```

### Ejecutar Clientes (múltiples terminales):
```bash
python chat-client.py 127.0.0.1 65432 cliente_01
python chat-client.py 127.0.0.1 65432 cliente_02
python chat-client.py 127.0.0.1 65432 cliente_03
```

### Funcionalidades implementadas:
1. **Username al conectar**: Cliente envía nombre automáticamente
2. **Formato mensajes**: `[cliente_01]: mensaje enviado`
3. **Broadcast**: Todos reciben los mensajes
4. **Auditoría**: Log en `audit.log` con timestamps
5. **Notificaciones**: Sistema notifica conexiones/desconexiones
...
### Salir del chat:
Escribir `quit` en el cliente

### Documentación teórica:
Ver `../AUDITORIA.md` para análisis completo de estrategia de auditoría.
