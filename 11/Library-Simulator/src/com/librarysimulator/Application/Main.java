...
public class Main extends Application {
    ...
    public static final Semaphore mPriorityMapMutex = new Semaphore(1, true);
    ...
    public static final Semaphore mOutChainSemaphore = new Semaphore(1, true);

    //semaphores for protecting the counter of the waiting student in each books
    public static final Semaphore mStudentCounterMutex = new Semaphore(1, true),
            mProfessorCounterMutex = new Semaphore(1, true);

    //a semaphore for the entry place
    public static final Semaphore mEntrySemaphore = new Semaphore(1, true);

    // a semaphore for getting the user book mutually
    public static final Semaphore mCurrentBookSemaphore = new Semaphore(1, true);
    ...
    public static final Semaphore mWaitingCounterMutex = new Semaphore(1, true),
            mTableCounterMutex = new Semaphore(1, true),
            mAvailableImportMutex = new Semaphore(1, true),
            mAvailableTableMutex = new Semaphore(1, true),
            mAvailableWaitingMutex = new Semaphore(1, true),
            mAvailableReturnMutex = new Semaphore(1, true);

    //semaphores for for the main places: chain , import, Table , waiting chairs and out chain
    public static final Semaphore mTableSemaphore = new Semaphore(12, true),
            mWaitingSemaphore = new Semaphore(3, true),
            mReturningSemaphore = new Semaphore(2, true),
            mChainSemaphore = new Semaphore(4, true),
            mImportSemaphore = new Semaphore(5, true);
    ...
    private void initChainAndWaitingSemaphores() {
        ...
        //initialising the list of the chain semaphores
        for (int i = 0; i < 4; i++) {
            mChainSemaphoresList.add(new Semaphore(1, true));
        }
    }
    ...
    private void initBooksMap(int semaphoreValue) {
        for (String book : mBooksObservableList) {
            mBooksSemaphoresMap.put(book, new Semaphore(semaphoreValue, true));
            mPriorityBooksSemaphoresMap.put(book, new Semaphore(semaphoreValue, true));
